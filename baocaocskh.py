...
def load_app_data(file):
    ...
    df_main['Tỷ lệ thực hiện qua App'] = pd.to_numeric(df_main['Tỷ lệ thực hiện qua App'], errors='coerce').fillna(0)
    if not df_total.empty:
        for col in ['STT', 'Số lượng KH quản lý', 'Số lượng đã thực hiện App']:
            df_total[col] = pd.to_numeric(df_total[col], errors='coerce').fillna(0).astype(int)
        df_total['Tỷ lệ thực hiện qua App'] = pd.to_numeric(df_total['Tỷ lệ thực hiện qua App'], errors='coerce').fillna(0)
...
def load_time_data(file):
    ...
    for col in ['STT', 'Số yêu cầu chuyển xử lý', 'Số lượng phiếu giải quyết trễ hạn']:
        df_main[col] = pd.to_numeric(df_main[col], errors='coerce').fillna(0).astype(int)
    df_main['Tỷ lệ trễ hạn'] = df_main['Tỷ lệ trễ hạn'].astype(str).str.replace('%','')
    df_main['Tỷ lệ trễ hạn'] = pd.to_numeric(df_main['Tỷ lệ trễ hạn'], errors='coerce').fillna(0)
    if not df_total.empty:
        for col in ['Số yêu cầu chuyển xử lý', 'Số lượng phiếu giải quyết trễ hạn']:
            df_total[col] = pd.to_numeric(df_total[col], errors='coerce').fillna(0).astype(int)
        df_total['Tỷ lệ trễ hạn'] = df_total['Tỷ lệ trễ hạn'].astype(str).str.replace('%','')
        df_total['Tỷ lệ trễ hạn'] = pd.to_numeric(df_total['Tỷ lệ trễ hạn'], errors='coerce').fillna(0)
    return df_main, df_total
...
def plot_bar(df, x_col, y_col, title, ylabel, percent=False, color='skyblue'):
    ...
    if percent:
        labels = [f"{v*100:.6f}%" for v in df[y_col]]
    else:
        labels = [f"{v*100:.6f}%" for v in df[y_col]]
    ...
    for _, row in df.iterrows():
        ...
        for i, col in enumerate(columns):
            ...
            if percent_col and col == percent_col:
                row_cells[i].text = f"{float(val)*100:.6f}"
...
def export_word_app(app_df, app_total, app_info, app_top3, app_bottom3, app_fig_top, app_fig_bot):
    ...
    doc.add_paragraph(
        f"Tỷ lệ thực hiện qua App trung bình đạt {app_info['avg_rate']*100:.6f}%. "
    ...
    doc.add_paragraph(
        f"Điện lực {app_top3.iloc[0]['Điện lực']} dẫn đầu với tỷ lệ {app_top3.iloc[0]['Tỷ lệ thực hiện qua App']*100:.6f}%. "
        f"Các điện lực top 3 đều có tỷ lệ trên {app_top3['Tỷ lệ thực hiện qua App'].min()*100:.6f}%."
    )
...
def export_word_time(time_df, time_total, time_info, time_top3, time_bottom3, time_fig_top, time_fig_bot):
    ...
    doc.add_paragraph(
        f"Tỷ lệ trễ hạn trung bình là {time_info['avg_late']*100:.6f}%. "
    ...
    doc.add_paragraph(
        f"Điện lực {time_top3.iloc[0]['Điện lực']} có tỷ lệ trễ hạn cao nhất ({time_top3.iloc[0]['Tỷ lệ trễ hạn']*100:.6f}%). "
        f"Các đơn vị top 3 đều có tỷ lệ trên {time_top3['Tỷ lệ trễ hạn'].min()*100:.6f}%."
    )
    doc.add_paragraph("Biểu đồ Top 3:")
    buf3 = BytesIO()
...
with tab1:
    ...
    if file_app:
        ...
        # Chỉ lấy tổng trên df_app (không cộng dòng Công ty)
        ...
        st.info(
            ...
            f"Tỷ lệ thực hiện qua App trung bình: {avg_rate*100:.6f}%\n\n"
            f"Tỷ lệ thực hiện qua App trung bình đạt {avg_rate*100:.6f}%. "
            "Một số điện lực đạt tỷ lệ cao, cho thấy hiệu quả truyền thông và hỗ trợ khách hàng tốt. "
            "Các đơn vị có tỷ lệ thấp cần tăng cường truyền thông, hỗ trợ kỹ thuật và khuyến khích khách hàng sử dụng App."
        )
        ...
        st.dataframe(
            ...
            .assign(**{'Tỷ lệ thực hiện qua App (%)': top3['Tỷ lệ thực hiện qua App'].apply(lambda x: f"{x*100:.6f}")})
            .drop('Tỷ lệ thực hiện qua App', axis=1)
            .reset_index(drop=True), hide_index=True)
        st.markdown(
            f"Điện lực {top3.iloc[0]['Điện lực']} dẫn đầu với tỷ lệ {top3.iloc[0]['Tỷ lệ thực hiện qua App']*100:.6f}%. "
            f"Các điện lực top 3 đều có tỷ lệ trên {top3['Tỷ lệ thực hiện qua App'].min()*100:.6f}%."
        )
        ...
        st.dataframe(
            ...
            .assign(**{'Tỷ lệ thực hiện qua App (%)': bottom3['Tỷ lệ thực hiện qua App'].apply(lambda x: f"{x*100:.6f}")})
            .drop('Tỷ lệ thực hiện qua App', axis=1)
            .reset_index(drop=True), hide_index=True)
        st.markdown(
...
with tab2:
    ...
    if file_time:
        ...
        st.info(
            ...
            f"Tỷ lệ trễ hạn trung bình: {avg_late*100:.6f}%\n\n"
            f"Tỷ lệ trễ hạn trung bình là {avg_late*100:.6f}%. "
            "Hầu hết các điện lực duy trì tỷ lệ trễ hạn thấp, nhưng vẫn còn một số đơn vị tỷ lệ cao hơn mức trung bình."
        )
        ...
        st.dataframe(
            ...
            .assign(**{'Tỷ lệ trễ hạn (%)': top3['Tỷ lệ trễ hạn'].apply(lambda x: f"{x*100:.6f}")})
            .drop('Tỷ lệ trễ hạn', axis=1)
            .reset_index(drop=True), hide_index=True)
        st.markdown(
            f"Điện lực {top3.iloc[0]['Điện lực']} có tỷ lệ trễ hạn cao nhất ({top3.iloc[0]['Tỷ lệ trễ hạn']*100:.6f}%). "
            f"Các đơn vị top 3 đều có tỷ lệ trên {top3['Tỷ lệ trễ hạn'].min()*100:.6f}%."
        )
        ...
        st.dataframe(
            ...
            .assign(**{'Tỷ lệ trễ hạn (%)': bottom3['Tỷ lệ trễ hạn'].apply(lambda x: f"{x*100:.6f}")})
            .drop('Tỷ lệ trễ hạn', axis=1)
            .reset_index(drop=True), hide_index=True)
        st.markdown(
